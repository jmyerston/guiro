# marcador.py v 0.6 marcador de relaciones a partir de patrones
...
def add_matches_to_stream(stream):
   patterns = "went|nsubj|ENTITY_ONE went|prep|to to|pobj|ENTITY_TWO"
   rules = [rule.split("|") for rule in patterns.split(" ")]
   print("Using this patterns:") 
   print(rules)
   constructed_pattern = construct_pattern(rules)
   matcher = DependencyMatcher(nlp.vocab)
   matcher.add("patrones", None, constructed_pattern)
   ...
   with open('went_to_relation.csv', 'w', newline='') as r:  # [went, Pythagoras, to, Delos]
       ...
       for eg in stream:
           ...
           for match_id, token_idxs in matches:
               for each_pattern in token_idxs: 
                   ...
                   branches = []
                   for x in matcher._tree:
                       branches.append(matcher._tree[x])
                   for k in branches[0][0]:
                       for rel, j in branches[0][0][k]:
...
def custom_dep_recipe(dataset, source):
    ...
    stream = add_matches_to_stream(stream)        # add custom patterns
